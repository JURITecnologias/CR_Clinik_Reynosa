    ...
    public function DatosUnidadDeUrgencia($month, $year)
    {
        ...
        SELECT 
            ...
            CASE
                ...
                ELSE '60 ˃'
            END AS rango_edad,
            -- Conteo por columna del Excel
            SUM(CASE WHEN JSON_CONTAINS(c.motivos_consulta, '\"aev\"') THEN 1 ELSE 0 END) AS accidentes_violencia,
            SUM(CASE WHEN JSON_CONTAINS(c.motivos_consulta, '\"uc\"') THEN 1 ELSE 0 END) AS urgencia_calificada,
            SUM(CASE WHEN JSON_CONTAINS(c.motivos_consulta, '\"unc\"') THEN 1 ELSE 0 END) AS urgencia_no_calificada,
            SUM(CASE WHEN JSON_CONTAINS(c.motivos_consulta, '\"trabajo_parto\"') THEN 1 ELSE 0 END) AS trabajo_parto,
            COUNT(*) AS total
        FROM consultas c
        JOIN pacientes p ON p.id = c.paciente_id
        WHERE 
            ...
            AND YEAR(c.fecha_consulta) = ".$year."
        GROUP BY p.sexo, rango_edad
        ORDER BY p.sexo DESC, rango_edad ASC;
        ";

        $resultados = DB::select($sql);

        $datos_filtrados = collect($resultados)->filter(function ($item) {
            $sumaMotivos = $item->accidentes_violencia + 
                       $item->urgencia_calificada + 
                       $item->urgencia_no_calificada + 
                       $item->trabajo_parto;
            return $sumaMotivos > 0;
        })->values();
